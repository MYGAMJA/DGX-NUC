"""Hylion v6 RL 학습 스크립트 — Newton 백엔드 (BG).

PhysX CPU 대신 Newton GPU 물리 가속을 사용.
DGX Spark (aarch64) 에서 PhysX는 CPU-only → Newton이 필수.

차이점 vs train_hylion_physx_BG.py:
  - isaaclab_newton.physics.NewtonCfg 적용
  - env_cfg.events.physics_material = None (PhysX-only API 비활성화)
  - 나머지 환경/PPO 설정 완전히 동일
...
사용법:
  ...
  PYTHONUNBUFFERED=1 LD_PRELOAD="/lib/aarch64-linux-gnu/libgomp.so.1" \\
    python /home/laba/DGX-NUC/sim/isaaclab/scripts/newton/train_hylion_newton_BG.py \\
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # Newton 백엔드 활성화 (PhysX CPU → Newton GPU)
    env_cfg.sim.physics = NewtonCfg()
    env_cfg.events.physics_material = None  # PhysX-only API — Newton과 충돌
